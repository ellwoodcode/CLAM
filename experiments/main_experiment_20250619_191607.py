...
def ssl_pretrain(args):
    ...
    dataset = SSLDataset(data_dir=os.path.join(args.data_root_dir, 'pt_files'), transform=transform)
    ...
    for epoch in range(args.ssl_epochs):
        ...
        print(f'Epoch [{epoch+1}/{args.ssl_epochs}], Loss: {total_loss/len(dataloader):.4f}')

    torch.save(model.encoder.state_dict(), os.path.join(args.results_dir, 'ssl_encoder.pth'))
...
parser.add_argument('--ssl_pretrain', action='store_true', default=True, help='enable self-supervised pretraining')
parser.add_argument('--ssl_epochs', type=int, default=50, help='number of epochs for SSL pretraining')
parser.add_argument('--ssl_batch_size', type=int, default=256, help='batch size for SSL pretraining')
parser.add_argument('--ssl_lr', type=float, default=1e-3, help='learning rate for SSL pretraining')
