...
class MultiScaleAttentionMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_scales):
            patch_feats = x[:, i, :]
            patch_emb = self.patch_encoders[i](patch_feats)
            scale_emb, _ = self.intra_scale_attention[i](patch_emb)
            scale_embeddings.append(scale_emb)
        scale_embeddings = torch.stack(scale_embeddings, dim=0)
        slide_embedding, _ = self.inter_scale_attention(scale_embeddings)
        logits = self.classifier(slide_embedding)
        out = {'logits': logits}

        if self.use_aux_loss:
            aux_logits = self.aux_head(scale_embeddings)
...
def train(datasets, cur, args):
    ...
    elif args.model_type == 'multi_scale_attention':
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            ...
            stop = validate_msa(cur, epoch, model, val_loader, args.n_classes, early_stopping, writer, loss_fn, args.results_dir)
...
def train_loop_msa(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None):
    model.train()
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        acc_logger.log(torch.topk(logits, 1, dim=1)[1], label)
        ...
        if 'aux_logits' in outputs:
            ...
            aux_loss = aux_loss_fn(outputs['aux_logits'], label)
            loss += args.aux_loss_weight * aux_loss

        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}'.format(batch_idx, loss_value))

        error = calculate_error(torch.topk(logits, 1, dim=1)[1], label)
        train_error += error
...
def validate_msa(cur, epoch, model, loader, n_classes, early_stopping=None, writer=None, loss_fn=None, results_dir=None):
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    val_loss = 0.
    val_error = 0.
    ...
    with torch.inference_mode():
        for batch_idx, (data, label) in enumerate(loader):
            ...
            prob[batch_idx] = F.softmax(logits, dim=1).cpu().numpy()
...
parser.add_argument('--num_scales', type=int, default=3, help='number of scales for multi-scale attention (default: 3)')
parser.add_argument('--attn_dim', type=int, default=128, help='attention dimension for multi-scale attention (default: 128)')
parser.add_argument('--use_aux_loss', action='store_true', default=True, help='use auxiliary loss in multi-scale attention')
parser.add_argument('--aux_loss_weight', type=float, default=0.5, help='weight for auxiliary loss in multi-scale attention (default: 0.5)')
