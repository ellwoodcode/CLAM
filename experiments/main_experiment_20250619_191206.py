...
class Generic_MIL_Dataset_Custom(Generic_MIL_Dataset):
    ...
    def get_slide_id(self, index):
        return self.slide_data[index]['slide_id']
...
def curriculum_select(slide_difficulty, epoch, curriculum_schedule):
    sorted_slides = sorted(slide_difficulty.items(), key=lambda x: x[1])
    threshold = curriculum_schedule[min(epoch, len(curriculum_schedule) - 1)]
...
def infer_patch_labels(model, loader):
    ...
    with torch.inference_mode():
        for i, (data, _) in enumerate(loader):
            ...
            slide_id = loader.dataset.get_slide_id(i * loader.batch_size)
...
def train(datasets, cur, args):
    ...
    for epoch in range(args.max_epochs):
        selected_slides = curriculum_select(slide_difficulty, epoch, args.curriculum_schedule)
        selected_loader = DataLoader([s for s in train_split if s['slide_id'] in selected_slides], batch_size=args.batch_size, shuffle=True)

        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            train_loop_clam(epoch, model, selected_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn, pseudo_labels, args.patch_loss_weight)
...
parser.add_argument('--curriculum_schedule', type=list, default=[30, 50, 70, 100], help='curriculum thresholds per epoch')
parser.add_argument('--label_refine_interval', type=int, default=5, help='number of epochs between pseudo-label updates')
parser.add_argument('--patch_loss_weight', type=float, default=0.1, help='weight for patch-level loss')
