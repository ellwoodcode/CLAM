...
def ssl_pretrain(args, encoder, train_loader):
    ...
    for epoch in range(args.ssl_epochs):
        ...
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            if data.dim() == 4 and data.size(1) != 3:
...
def get_ssl_loader(dataset, batch_size):
    ...
    return DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=4, collate_fn=collate_fn_fixed_size)

def collate_fn_fixed_size(batch):
    # Ensure all tensors in the batch have the same size
    max_size = max([item[0].size(0) for item in batch])
    padded_batch = []
    for item in batch:
        data, label = item
        if data.size(0) < max_size:
            padding = torch.zeros(max_size - data.size(0), data.size(1), device=data.device)
            data = torch.cat((data, padding), dim=0)
        padded_batch.append((data, label))
...
# SSL related arguments
parser.add_argument('--ssl_pretrain', action='store_true', default=True, help='Enable self-supervised pretraining stage')
parser.add_argument('--ssl_epochs', type=int, default=100, help='Number of epochs for SSL pretraining')
parser.add_argument('--ssl_batch_size', type=int, default=64, help='Batch size for SSL pretraining')
parser.add_argument('--ssl_proj_dim', type=int, default=128, help='Dimension of SSL projection head')
parser.add_argument('--ssl_temp', type=float, default=0.5, help='Temperature parameter for contrastive loss')
